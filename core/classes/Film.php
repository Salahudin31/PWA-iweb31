...
class Film
{
	...
	function add_film($kd_film,$kd_genre,$kd_negara,$kd_kategori,$kd_kualitas,$kd_rating,$tgl_r,$jdl_film,$img,$link_film,$durasi,$desk){
		$query = $this->db->prepare("INSERT INTO `film` (`$kode_film`,`$kode_genre`,`$kode_negara`,`$kode_kategori`,`$kode_kualitas`,`$kode_rating`,`$tgl_rilis`,`$judul_film`,`img`,`$link_film`,`$durasi`,`$desk`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
		...
		$query->bindValue(8,$jdl_film);
		$query->bindValue(9,$img);
		$query->bindValue(10,$link_film);
		$query->bindValue(11,$durasi);
		$query->bindValue(12,$desk);
		try{
			$query->execute();
		}catch(PDOException $e){
			die($e->getMessage());
	...
	function update_film($id_film,$kd_film,$kd_genre,$kd_negara,$kd_kategori,$kd_kualitas,$kd_rating,$tgl_r,$jdl_film,$img,$link_film,$durasi,$desk){
		$query = $this->db->prepare("UPDATE `film` SET `kode_film` = ?, `kode_genre` = ?, `kode_negara` = ?, `kode_kategori` = ?, `kode_kualitas` = ?, `kode_rating` = ?, `tgl_rilis` = ?, `judul_film` = ?, `img`= ?, `link_film` = ?, `durasi` = ?, `desk` = ? WHERE `id_film` = ?");
		$query->bindValue(13,$id_film);
		$query->bindValue(1,$kd_film);
		$query->bindValue(2,$kd_genre);
		$query->bindValue(3,$kd_negara);
		$query->bindValue(4,$kd_kategori);
